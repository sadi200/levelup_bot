...
🔢 <b>ID:</b> ${PROJECT_CONFIG.id}
⏰ <b>Duration:</b> ${config.duration} min
...
const sendMessageNotification = async (messageNumber, content, channelId, token, status = "online") => {
  // যদি detailed logs disable থাকে, শুধু প্রতি ৫তম message-এ notification পাঠাবে
  if (!ENABLE_DETAILED_LOGS && messageNumber % 5 !== 0) return true;
  
  const truncatedContent = content.length > 35 ? content.substring(0, 35) + '...' : content;
  const apiStatus = GEMINI_API_KEY ? "ON" : "OFF";
...
📛 <b>Project:</b> ${PROJECT_CONFIG.name}
🔢 <b>ID:</b> ${PROJECT_CONFIG.id}
⏱️ <b>Runtime:</b> ${durationMinutes.toFixed(1)} min
